...
| [GHSA-v6wh-96g9-6wx3](https://github.com/advisories/GHSA-v6wh-96g9-6wx3) | vite (medium) | NTLMv2 hash disclosure via UNC path in vite's dev server, Windows only. Production runs `next start` on Alpine Linux; the vite dev server is never run on Windows in this project. |
| [GHSA-h67p-54hq-rp68](https://github.com/advisories/GHSA-h67p-54hq-rp68) | js-yaml (medium) | Quadratic-complexity DoS via YAML merge keys (`<<`) in js-yaml. js-yaml is a transitive toolchain dependency (used by build tools to parse their own config files). No user-supplied YAML is ever passed through js-yaml at runtime or build time in this project. |
| [GHSA-4x5r-pxfx-6jf8](https://github.com/advisories/GHSA-4x5r-pxfx-6jf8) | @babel/core (low) | Arbitrary file read via a `sourceMappingURL` comment in JavaScript processed by Babel. Babel is invoked exclusively on this project's own source files at build time — no untrusted input ever reaches Babel, so the vulnerable code path cannot be triggered. |
